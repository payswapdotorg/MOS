...
| MKT-015 | IN FLIGHT | dispatch mkt-015k (chat 88eb965c) via the VPN-era send-button pipeline; prompt verified server-side 18:07Z; streaming long implementation turns (Experiment model is a large work item). |
| MKT-031 | IN FLIGHT | dispatch mkt-031d (chat a7e217d1) after the original chat (fc11e689) became unrecoverable (dangling pending turn silently dropped every send); turns flowing rapidly since 18:25Z. |
