...
// MKT-013: evidence owner context for evidence-scoped access checks.
...
// MKT-015: /experiments canonical owner context (experiment design records).
...
/**
 * Sandbox-lifecycle access check (MKT-012).
 *
 * Step 1 loads the canonical Sandbox owner context (sandbox row → its
 * owning Workspace → the owning Client → the owning Agency, all from
 * durable state): an unknown identifier OR a tombstoned boundary is a
 * uniform 404 (indistinguishable). Step 2 authorizes the caller against
 * the agency that OWNS the sandbox's Workspace — the SAME durable
 * membership authority as every other scoped check (no second
 * authorization authority):
 *
 *   - service principal and platform administrators pass;
 *   - a caller with NO membership in the owning agency gets the SAME 404 as
 *     for an unknown sandbox — a foreign sandbox id is not a
 *     traversal/existence oracle (cross-tenant posture);
 *   - a suspended membership or disabled identity never authorizes (403);
 *   - `roles` optionally restricts to specific agency roles (403 otherwise).
 *
 * A Sandbox ID is NEVER itself an authorization credential — it only
 * selects WHICH durable ownership chain gets resolved.
 */
export async function requireSandboxAccess(
  ...
  if (membership === undefined) {
    // Hard boundary: not a member of the agency owning the sandbox's
    // WORKSPACE → indistinguishable from an unknown sandbox (uniform 404,
    // no cross-tenant oracle; rejection BEFORE any dependent traversal).
    throw new NotFoundError('sandbox', sandboxId);
  }
  if (membership.membershipStatus !== 'active') {
    throw new ForbiddenError('Active membership in the sandbox workspace agency required');
  }
  if (roles !== undefined && !roles.includes(membership.role)) {
    throw new ForbiddenError('This operation requires a different agency role');
  }
...
/**
 * Evidence-scoped access check (MKT-013, EVID-AC-01/03 posture).
 *
 * Step 1 loads the canonical Evidence owner context (evidence record → its
 * Client via /clients resolveClientOwnership → owning Agency, plus the
 * scoped Workspace row when workspace-scoped, all from durable state): an
 * unknown identifier, a record owned by a deleted-tombstone Client, or a
 * record of another Agency's Client is a UNIFORM 404 (indistinguishable —
 * no traversal/existence oracle; the immutable ledger is never an oracle
 * for foreign identifiers). Step 2 authorizes the caller against the agency
 * that OWNS the record's Client — the SAME durable membership authority as
 * every other scoped check (no second authorization authority):
 *
 *   - service principal and platform administrators pass;
 *   - a caller with NO membership in the owning agency gets the SAME 404 as
 *     for an unknown record — a foreign evidence id is not a
 *     traversal/existence oracle (cross-tenant posture);
 *   - a suspended membership or disabled identity never authorizes (403);
 *   - `roles` optionally restricts to specific agency roles (403 otherwise
 *     — the explicit supersede command requires owner|admin).
 *
 * An Evidence ID is NEVER itself an authorization credential — it only
 * selects WHICH durable ownership chain gets resolved.
 */
...
/**
 * MKT-015: canonical experiment access check. Resolves the experiment's
 * owning Client through the /experiments module's canonical owner chain
 * (experiment → /clients → /agencies) from durable state BEFORE any
 * dependent traversal, then enforces the same hard-boundary posture as
 * evidence and metric observations: a foreign experiment identifier is
 * indistinguishable from an unknown one (uniform 404 — no cross-tenant
 * oracle), while intra-tenant failures (suspended membership, wrong role)
 * are 403s exactly like every other scoped authority.
 */
export async function requireExperimentAccess(
  ...
  experimentId: string,
  roles?: ReadonlyArray<AgencyRoleKey>,
): Promise<ExperimentOwnerContext> {
  ...
  if (membership.membershipStatus !== 'active') {
    throw new ForbiddenError('Active membership in the experiment client agency required');
  }
  if (roles !== undefined && !roles.includes(membership.role)) {
    throw new ForbiddenError('This operation requires a different agency role');
  }
...
/**
 * MKT-016: canonical Learning access check. Resolves the learning's
 * owning Client through the /learnings module's canonical owner chain
 * (learning → /clients → /agencies) from durable state BEFORE any
 * dependent traversal, then enforces the same hard-boundary posture as
 * evidence, metric observations and experiments: a foreign learning
 * identifier is indistinguishable from an unknown one (uniform 404 — no
 * cross-tenant oracle), while intra-tenant failures (suspended
