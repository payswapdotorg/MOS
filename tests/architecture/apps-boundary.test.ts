...
test('MKT-047 provision: the checker enforces /apps with an EMPTY matrix allowance; the spec-parsed set stays 37', () => {
  ...
  // /social-accounts (registered by the MKT-055 delivery — the v1.6
  // Social Account and OAuth Connection Model) — all per the disclosed
  // promotion precedent. /apps itself is NOT in
...
test('MKT-047 AC-7: the expected-migration list carries 037 in numeric position; MKT-046 appends 040 (the shared infra-adapters surface)', () => {
  ...
  // MKT-050 appends 042 after 040; MKT-052 appends 044 after 042;
  // MKT-055 appends 046 after 044 (the numbers are PRE-ASSIGNED —
  // 039/041/043/045 reserved-but-unused by the no-migration/sibling
  // deliveries): 037 and 038 keep their numeric positions.
