...
test('MKT-047 AC-7: the expected-migration list carries 037 in numeric position; MKT-046 appends 040 (the shared infra-adapters surface)', () => {
  ...
  // Order-based tail assertions (conflict-tolerant under sibling
  // appends): each PRE-ASSIGNED migration holds its numeric position
  // relative to its neighbors. The MKT-068 delivery appends
  // 047_notification_delivery.sql after 046 (the number is PRE-ASSIGNED
  // to that Work Item; 048/049 are reserved for siblings).
  const position = (name: string) => migrationsOnDisk.indexOf(name);
  assert.ok(position('037_apps.sql') >= 0);
  assert.ok(position('037_apps.sql') < position('038_app_installs.sql'));
  assert.ok(position('038_app_installs.sql') < position('040_sales_continuity.sql'));
  assert.ok(position('040_sales_continuity.sql') < position('042_app_marketplace.sql'));
  assert.ok(position('042_app_marketplace.sql') < position('044_app_metering.sql'));
