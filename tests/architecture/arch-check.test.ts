...
test('frozen dependency matrix is parsed from the frozen spec documents', () => {
  ...
  // The MKT-068 additive matrix line (the v1.6-frozen row: the
  // /notification-delivery Notification Delivery Plane over the
  // /notifications boundary, the /policies gates and the /credentials
  // vault):
...
test('negative fixture: structure violations are rejected (unknown module dir, missing public entry, stray file, missing frozen modules)', () => {
  ...
  // /notification-delivery registered by the MKT-068 delivery, both per
