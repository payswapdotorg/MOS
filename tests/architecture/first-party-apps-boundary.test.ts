...
test('AC-3: ZERO SQL, ZERO tables, ZERO database dependency in the module (no durable state)', () => {
  ...
  // deliveries append 047, 048 and 049 after 046, and the MKT-056
  // social-adapter-contract delivery appends 050 (the /social-accounts
  // capability-plane extension — the sibling-promotion precedent; this
  // delivery still owns NO migration).
  assert.deepEqual(migrations.slice(-7), ['044_app_metering.sql', '045_growth_missions.sql', '046_social_accounts.sql', '047_notification_delivery.sql', '048_product_intelligence.sql', '049_commerce_capabilities.sql', '050_social_adapter_contract.sql'], 'the migration list tail carries the sibling promotions only');});
