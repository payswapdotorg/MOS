...
test('AC-3: ZERO SQL, ZERO tables, ZERO database dependency in the module (no durable state)', () => {
  ...
  // MKT-052 metering migration, and the MKT-071 /integrations commerce
  // extension appends 049 after 046 (047/048 belong to sibling deliveries;
  // the sibling-promotion precedent — this delivery still owns NO migration).
  assert.deepEqual(migrations.slice(-4), ['044_app_metering.sql', '045_growth_missions.sql', '046_social_accounts.sql', '049_commerce_capabilities.sql'], 'the migration list tail carries the sibling promotions only');});
