...
test('AC-3: ZERO SQL, ZERO tables, ZERO database dependency in the module (no durable state)', () => {
  ...
  assert.deepEqual(migrations.slice(-4), ['044_app_metering.sql', '045_growth_missions.sql', '046_social_accounts.sql', '047_notification_delivery.sql'], 'the migration list tail carries the sibling promotions only');});
...
test('AC-6: the composition root wires the module with NO database handle (the no-migration posture)', () => {
  ...
  // MKT-055 /social-accounts and MKT-068 /notification-delivery deliveries
