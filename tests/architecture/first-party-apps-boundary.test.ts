...
test('AC-3: ZERO SQL, ZERO tables, ZERO database dependency in the module (no durable state)', () => {
  ...
  // MKT-052 metering migration; the MKT-071 delivery appends
  // 049_commerce_capabilities.sql (the PRE-ASSIGNED commerce-capabilities
  // migration of the /integrations boundary — 047/048 are PRE-ASSIGNED to
  // sibling deliveries and are not in this tree; the sibling-promotion
  // precedent — this delivery still owns NO migration).
  assert.deepEqual(migrations.slice(-3), ['045_growth_missions.sql', '046_social_accounts.sql', '049_commerce_capabilities.sql'], 'the migration list tail carries the sibling promotions only');});
