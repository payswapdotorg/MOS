...
test('MKT-053 AC-9 static: the disclosed spec registration exists — §6 line + the matrix row + the authority-notes bullet + 045 in numeric position', () => {
  ...
  // it after this one per the pre-assignment; the MKT-069 delivery
  // appends 048 after 046 — the numeric ORDER is the invariant; 047/049
  // are reserved for sibling deliveries and append between 046 and 048
  // when they land).
  const migrationsOnDisk = readdirSync(src('platform', 'db', 'migrations'))
    .filter((name) => name.endsWith('.sql'))
    .sort();
  assert.ok(migrationsOnDisk.includes('045_growth_missions.sql'));
