...
test('MKT-056: the sanctioned adapter subtree exists; MKT-057/MKT-058/MKT-059 add the concrete platform adapters (youtube, instagram, facebook-pages) beside the re-export shim', () => {
  ...
  // The MKT-059 re-pin: the facebook-pages subtree is a single adapter
...
test('MKT-056: the disclosed composition seams exist; MKT-057 registers the FIRST first-party platform adapter (the seam keeps overriding for tests)', () => {
  ...
  // MKT-059 re-pin: the THIRD first-party platform adapter (Facebook
  // Pages) registers as production DATA through the same pattern.
  assert.ok(
